...
varFormat={1:"A", 2:"HEX", 3:"COMMA", 4:"DOLLAR", 5:"F", 6:"IB", 7:"PIBHEX", 8:"P", 9:"PIB", 10:"PK", 11:"RB", 
           ...
           34:"CCB", 35:"CCC", 36:"CCD", 37:"CCE", 38:"EDATE", 39:"SDATE"}
...
class BaseBinary(object):
    ...
    def getcaseNumForWrite(self):
        return self.caseNumForWrite
    
    def getcaseLenForWrite(self):
        return self.caseLenForWrite
    
    def getcacheForWrite(self):
        return self.cacheForWrite
    ...
    def getcaseCacheSize(self):
        return self.caseCacheCount
    ...
    def openSendDataFile(self):
        self.senddatafile = open(self.senddatatempfile.name, 'rb')
        
    def writeSendDataFile(self, data):
        self.senddatatempfile.write(data)
        
    def closeSendDataFile(self):
        if not self.senddatafile.closed:
            self.senddatafile.close()
            self.senddatafile = None
    ...
    def closeTempDataset(self, datasetName):
        cmd = "DATASET CLOSE %s." % datasetName
                    ...
                    if self.userMissing[i][1] is not None:
                        if self.varType[i] == 0:
                            ...
                            if 0 != self.userMissing[i][0]:
                                if self.userMissing[i][2]>currentcase[i]>self.userMissing[i][1]:
                                    currentcase[i] = None
                        else:
