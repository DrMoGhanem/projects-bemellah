...
import sys
import time, os, re, fnmatch, glob, copy, random, textwrap
...
#     21-jan-2-11  Add delEmptyVars function
#    08-jun-12  Reimplement FindEmptyVars for greater efficiency
#    16-oct-13  Protect FindEmptyVars from huge numbers of deletions
#    03-dec-13  bugfix in FindEmpyVars for case of unnamed dataset
#    09-dec-13  speed up FindUnlabelledValues
#    28-jan-14  handle huge variable lists in delEmptyVars
#    10-mar-14  add SpssWrap class
...
def FindEmptyVars(vars=None, delete=False, alpha=True):
    ...
    if not isinstance(vars, spssaux.VariableDict):
            vars = spssaux.VariableDict(vars)
    varlist = vars.variables
    inputds = spss.ActiveDataset()
    # ensure that the active dataset is named
    nameassigned = inputds == "*"
    if nameassigned:
        inputds = "D" + str(random.random())
        spss.Submit("""DATASET NAME %s""" % inputds)

    outputds = "A" + str(random.random())
    cmd = [r"""DATASET DECLARE %(outputds)s.
AGGREGATE OUTFILE=%(outputds)s""" % locals()]
    
    # build aggregate variable specifications
    kept = []
    for v in vars:
        trfname = "V" + str(random.random()+0.01)
        inputname = v.VariableName
        if v.VariableType > 0:
            if not alpha:
                continue
            cmd.append(r"""/%(trfname)s_min = MIN(%(inputname)s) /%(trfname)s_max = MAX(%(inputname)s)""" % locals())
        else:
            cmd.append(r"""/%(trfname)s = NUMISS(%(inputname)s)""" % locals())
        kept.append(inputname)

    killlist = []
    if kept:
        spss.Submit("\n".join(cmd))
        spss.Submit("DATASET ACTIVATE %s" % outputds)
        curs = spss.Cursor()
        results = curs.fetchone()  # one tuple of values
        curs.close()
        spss.Submit("DATASET ACTIVATE %s" % inputds)
        spss.Submit("DATASET CLOSE %s" % outputds)
        ...
        for v in vars:
            if v.VariableType > 0:
                if not alpha:
                    continue
                if results[vindex].rstrip() == "" and results[vindex+1].rstrip() == "":
                    killlist.append(v.VariableName)
                vindex += 2
            else:
                if results[vindex] == ncases:
                    killlist.append(v.VariableName)
                vindex += 1
        if delete and len(killlist) > 0:
            spss.Submit("DELETE VARIABLES " + "\n".join(killlist))
    if nameassigned:
        spss.Submit("DATASET CLOSE %s" % inputds)
    return killlist
...
def FindUnlabelledValues(vardict):
    ...
    # Use the Dataset class if possible for faster label access
    try:
        with spss.DataStep():
            ds = spss.Dataset()
            for v in vardict:
                lbls = ds.varlist[v.VariableName].valueLabels.data
                if len(lbls) > 0:
                    varstocheck.append(v.VariableName)
                    if v.VariableType > 0:
                        lbls = [k.rstrip() for k in lbls.keys()]
                    else:
                        lbls = [k for k in lbls.keys()]
                    labelledvalues.append(set(lbls))
                    allvalues.append(set())                
            ds.close()
    except:   # if we can't use a datastep, try the old, slow way
        try:
            for v in vardict:
...
# delete all empty numeric or string(blank) variables
# JKP, IBM SPSS
# 1/19/2011
import spss, spssdata

def delEmptyVars(varlist="ALL"):
    """Find numeric variables with all values sysmis and string variables all blank and delete them
    return list of variables deleted
    
    If varlist is specified, the checks are limited to the listed variables.
    varlist can be a sequence of names or a blank-separated string"""
    
    curs = spssdata.Spssdata(varlist, convertUserMissing=False)
    s = set(curs.varnames())
    ...
    curs.CClose()
    if len(s) > 0:
        varlist = textwrap.wrap(" ".join(s), width=100, 
            break_long_words=False, break_on_hyphens=False)
        spss.Submit("DELETE VARIABLES " + "\n".join(varlist))
    return list(s)

# This class wraps SPSS syntax-like strings safely
# Usage: create an SpssWrap object; then use its wrap method
# for each Submit block
# Example:
# tw = SpssWrap()
# cmd="""SUMMARIZE
#/TABLES=lots of variable names BY gender
#/FORMAT=VALIDLIST NOCASENUM TOTAL LIMIT=100
#/TITLE="Case ''Summaries"+
#" more stuff"
#/FOOTNOTE 'The caption ""is"" hello'
#/CELLS=COUNT MEDIAN RANGE."""
# cmdlist = tw.wrap(cmd)
# spss.Submit(cmdlist)
