...
class BasePivotTable(object):
    ...
    def __getitem__(self, categories):
        ...
        map(CellText._CellText__CheckType,categories)

        if self.cells.has_key(tuple(categories)):
    ...
    """
        ...
        if cells is not None:
            nRows = len(cells)
            if nRows > 0:
                if not isinstance(cells[0],basestring):
                    try:
...
class Dimension(object):
    ...
    def __SetCategory(self,category):
        ...
        if 0 == category.data["type"]: #Number
            ...
            if isinstance(category.data["value"],basestring):
    ...
    def __SetCell(self,cell):
        ...
        if 0 == cell.data["type"]: #Number
            ...
            if isinstance(cell.data["value"],basestring):
...
class CellText(object):
    ...
    def __ToCellText(obj):
        ...
        elif isinstance(obj, basestring): return CellText.String(obj)
        elif isinstance(obj, (int,long,float,time.struct_time,datetime.date)): return CellText.Number(obj)
        else:
            error.SetErrorCode(1004)
            if error.IsError():
                raise SpssError, error
