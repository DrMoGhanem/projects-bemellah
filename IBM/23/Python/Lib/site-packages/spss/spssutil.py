...
def isDateVariable(var):
    ...
    if isinstance(var, basestring):
        return var[:3] in ['DAT','ADA','EDA','JDA','SDA','QYR','MOY','WKY','DAT']
    elif isinstance(var, int):
        return var in set([20,22,23,24,28,29,30,38,39])
...
def extrapath():
    ...
    if os.path.exists(spssxdcfg):
        ...
        spssMajVersion = config.get('version','SpssxdVersionMajor')
        ...
        if sys.platform.lower() == "win32":
            try:
                ...
                windll.shell32.SHGetFolderPathW(0, 0x001c, 0, 0, pathBuf)
                try:
                    winAppPath = pathBuf.value.encode(locale.getdefaultlocale()[1])
                except UnicodeEncodeError:
                    winAppPath = pathBuf.value.encode("utf-8")
            except ImportError:
                import platform
                winAppPath = os.path.expanduser("~")
                if platform.release().lower() != "xp":
                    if os.environ.has_key("LOCALAPPDATA"):
                        winAppPath = os.getenv("LOCALAPPDATA")
                    else:
                        winAppPath = os.sep.join([winAppPath, "AppData\\Local"])
                else:
                    winAppPath = os.sep.join([winAppPath, "Local Settings\\Application Data"])                
        ...
        if sys.platform.lower() == "darwin":
            ...
            spssExtension = os.sep.join([spssHome, "extensions"])
        if os.path.isdir(spssExtension) and spssExtension not in sys.path:
            sys.path.insert(0, spssExtension)
